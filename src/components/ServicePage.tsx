    ...
    <>
      ...
      <div style={{ position: 'relative', height: 420, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          padding: '0 2rem', paddingTop: 72,
        }}>
          <p style={{ fontSize: 12, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#b8974a', marginBottom: 12 }}>
            Paslaugos
          </p>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(2rem, 4vw, 3rem)', color: 'white', fontWeight: 400, marginBottom: 12 }}>
            {title}
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 17, maxWidth: 500 }}>{subtitle}</p>
        </div>
      </div>
      ...
      {relatedServices && (
        <section style={{ padding: '4rem 2rem', background: '#f5f0e8' }}>
          <div style={{ maxWidth: 800, margin: '0 auto' }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, marginBottom: 24, color: '#1a1a18' }}>
              Kitos paslaugos
            </h2>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
              {relatedServices.map(s => (
                <Link key={s.href} href={s.href} style={{
                  border: '1px solid rgba(184,151,74,0.4)', color: '#b8974a',
                  padding: '10px 20px', borderRadius: 2, fontSize: 14, transition: 'all 0.2s',
                }}>
                  {s.label}
                </Link>
              ))}
